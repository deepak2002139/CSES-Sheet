...
ll solve(vl &nums,ll x,int curr_sum)
{
    
    if(x<curr_sum)return 0;
    
    dp[curr_sum]=0;
    for(int j=0;j<nums.size();j++){
        if(nums[j]<=curr_sum)
        dp[curr_sum]+=dp[curr_sum-nums[j]];
    }
    return dp[curr_sum]+solve(nums,x,curr_sum+1);
    
}
// ll dp[1000001];
// ll solve(vl &nums, ll x)
// {
//     ll n=nums.size();
//     dp[0]=1;
//     for(ll i=0;i<=x;i++)
